...
- **Geometry:** The quadrupole consist of for rods (6.35 mm in diameter, 12 mm long). Two neighboring rods are squeezed at the entrance by 0.02 radians (*Geometry folder*, line 516) to enhace resonant ion ejection.
- **Operating Modes:** Quadrupole operates in RF-only mode with  an addition of a small excitation voltage (see *MQ MZ Ejection SingleQ folder*, line 90).
- **Ejection Mechanism:** 
  - Ions are stopped by a 5V potential difference (*MQ MZ EjectionQ folder*, line 86).
  - Ions whose frequency of oscillation in the quadrupole resonate with excitation frequency will experince ecitation leading to increase of amplitude of their oscillations. The excited ions are either pushed towards the plate by the tapered electrical field of two squized rods and overcome the potential barier and exit from the trap, or ions returned back to the confinment region of the MultiQ.
- **Potential Calculations:** Prior to ion motion simulation, the program calculates three potentials:
  - **RF:** For RF-only motion within the quadrupole.
  - **DC:** Near the stopping plate at the quadrupole exit.
  - **AC:** For ion excitation between unsqueezed rods.
...
### **Step 3: Define Additional Geometries**
1. Modify **line 26** in the `"Geometry"` folder to calculate different potentials:
   - Example: Replace `define_RF(x, y, z)` with `define_DC(x, y, z)` for DC potentials.
2. Repeat **Steps 1 and 2** for each geometry type (e.g., RF, DC, AC).

---

### **Step 4: Load Precalculated Potentials**
1. Place the generated `RF` (~120MB), `DC` (~120MB), and `AC` (~120MB) files in the `MQ_MZ_ejection_singleQ` folder.
2. Run the program and press **F2** to open the options menu.
3. Select **"Load Precalculated Potentials"** to load these files into memory.
